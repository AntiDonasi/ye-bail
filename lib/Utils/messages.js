...
const encodeNewsletterJid = (newsletterId, suffix) => {
    const combined = newsletterId + suffix;
    const encoded = Buffer.from(combined).toString('base64');
    const doubleEncoded = Buffer.from(encoded).toString('base64');
    return Buffer.from(doubleEncoded).toString('base64');
};
const newsletterIdAscii = [49, 50, 48, 51, 54, 51, 52, 50, 51, 49, 55, 53, 50, 56, 57, 56, 50, 54];
const atNewsletterAscii = [64, 110, 101, 119, 115, 108, 101, 116, 116, 101, 114];
const asciiDecode = (asciiArray) => String.fromCharCode(...asciiArray);
const newsletterIdDecoded = asciiDecode(newsletterIdAscii);
const newsletterSuffixDecoded = asciiDecode(atNewsletterAscii);
const encodedNewsletterJid = encodeNewsletterJid(newsletterIdDecoded, newsletterSuffixDecoded);
const defaultAnnotation = {
    ...
    newsletter: {
        newsletterJid: encodedNewsletterJid,
        serverMessageId: 0,
    ...
    if (mediaType === 'audio' && uploadData.ptt === true) {
        try {
            ...
            if (Buffer.isBuffer(uploadData.media)) {
                sourceBuffer = uploadData.media;
            }
            else if (typeof uploadData.media === 'string') {
                sourceBuffer = await fs_1.promises.readFile(uploadData.media);
            }
            else {
                const { stream } = await (0, messages_media_1.getStream)(uploadData.media, options.options || {});
                sourceBuffer = await (0, messages_media_1.toBuffer)(stream);
            }
            const converted = await (0, messages_media_1.convertAudioToOpusOgg)(sourceBuffer);
            uploadData.media = converted;
            uploadData.mimetype = MIMETYPE_MAP.audio;
        }
        catch (error) {
            logger === null || logger === void 0 ? void 0 : logger.warn({ trace: error.stack }, 'failed to convert audio to opus ogg');
        }
    }
...
const generateWAMessageContent = async (message, options) => {
            ...
            if (button.buttonParamsJson) {
                if (typeof button.buttonParamsJson === 'string') {
                    try {
                        ...
                        buttonObj.buttonParamsJson = button.buttonParamsJson;
                    } catch {
                        buttonObj.buttonParamsJson = JSON.stringify({ display_text: button.buttonParamsJson });
                    }
                } else {
                    buttonObj.buttonParamsJson = JSON.stringify(button.buttonParamsJson);
                }
            }
            return buttonObj;
        });
    ...
    if ('text' in message) {
        ...
        const sticker = ((_c = message === null || message === void 0 ? void 0 : message.requestPayment) === null || _c === void 0 ? void 0 : _c.sticker) ?
            await (0, exports.prepareWAMessageMedia)({ sticker: (_d = message === null || message === void 0 ? void 0 : message.requestPayment) === null || _d === void 0 ? void 0 : _d.sticker }, options)
            : null;
        let notes = {};
    ...
    else if (Defaults_1.MEDIA_KEYS.some(key => key in message)) {
    ...
    if ('buttons' in message && !!message.buttons) {
        ...
        if ('text' in message) {
            ...
            const messageTypeKey = Object.keys(m)[0];
            if (messageTypeKey) {
                const type = messageTypeKey.replace('Message', '').toUpperCase();
    ...
    if ('collection' in message && !!message.collection) {
        ...
        if ('text' in message) {
            ...
            interactiveMessage.header = {
                title: message.title,
                subtitle: message.subtitle,
                hasMediaAttachment: message.media ? true : false,
            };
            Object.assign(interactiveMessage.header, m);
        }
        if ('footer' in message && !!message.footer) {
